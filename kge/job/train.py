...
class TrainingJobNegativeSampling(TrainingJob):
    ...
    def _process_subbatch(
        ...
        for slot in [S, P, O]:
            ...
            if self.config.get("train.loss") == "bce_freq":
                all_freq = self.dataset._freqs[self.train_split]["all"]
                slot_freqs = self.dataset._freqs[self.train_split][SLOT_STR[slot]]
                negative_samples = batch_negative_samples[slot].samples(indexes=subbatch_slice)
                label_freqs = slot_freqs[negative_samples.squeeze()].reshape(negative_samples.shape[0], -1).to(self.device)
                loss_value_torch = (
                        self.loss(scores, labels[slot], num_negatives=num_samples, label_freqs=(label_freqs/all_freq)) / batch_size
                )
            elif self.config.get("train.loss") == "kl_freq":
                # not implemented
                assert(False)
            else:
                loss_value_torch = (
...
class TrainingJob1vsAll(TrainingJob):
    ...
    def __init__(
        ...
        self.label_smoothing = config.check_range(
            "train.label_smoothing", float("-inf"), 1.0, max_inclusive=False
        )
        if self.label_smoothing < 0:
            if config.get("train.auto_correct"):
                config.log(
            ...
            if config.get("loss") != "ce" and config.get("loss") != "kl":
                raise Exception(
                    "Currently, label smoothing only works with KL or CE loss."
                )
            if config.get("train.auto_correct"):
                # just to be sure it's used correctly
                config.log(
    ...
    def _process_subbatch(
        ...
        if self.loss.__class__.__name__ == "KLDivWithSoftmaxKgeLoss":
            self.loss._celoss.update_smooth(self.label_smoothing, self.num_entities)
        if self.config.get("train.loss") == "kl_freq":
            label_freqs = []
            sp_ids = []
            for batch_id, label_index in enumerate(triples[:, 2]):
                sp = (int(triples[batch_id][0]), int(triples[batch_id][1]))
                sp_ids.append(self.dataset._tuple2id[self.train_split]["sp_"][sp])
            sp_ids = torch.LongTensor(sp_ids)
            sp_freqs = self.dataset._freqs[self.train_split]["sp_"][sp_ids]
            o_freqs = self.dataset._freqs[self.train_split]["o"][triples[:, 2]]
            label_freqs = (sp_freqs / o_freqs).to(self.device)
            loss_value_sp = self.loss(scores_sp, triples[:, 2], label_freqs=label_freqs) / batch_size
        elif self.config.get("train.loss") == "bce_freq":
            # not implemented
            assert(False)
        else:
        ...
        if self.config.get("train.loss") == "kl_freq":
            label_freqs = []
            po_ids = []
            for batch_id, label_index in enumerate(triples[:, 0]):
                po = (int(triples[batch_id][1]), int(triples[batch_id][2]))
                po_ids.append(self.dataset._tuple2id[self.train_split]["_po"][po])
            po_ids = torch.LongTensor(po_ids)
            po_freqs = self.dataset._freqs[self.train_split]["_po"][po_ids]
            s_freqs = self.dataset._freqs[self.train_split]["s"][triples[:, 0]]
            label_freqs = (po_freqs / s_freqs).to(self.device)
            loss_value_po = self.loss(scores_po, triples[:, 0], label_freqs=label_freqs) / batch_size
        elif self.config.get("train.loss") == "bce_freq":
            # not implemented
            assert(False)
        else:
