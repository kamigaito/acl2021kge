...
class LabelSmoothingLoss(torch.nn.Module):
    ...
    def update_smooth(self, smoothing, classes):
    ...
    def forward(self, pred, target, **kwargs):
        ...
        if "label_freqs" in kwargs:
            if self.smoothing > 0.0:
                # not implemented
                assert(False)
            return torch.sum(kwargs["label_freqs"] * torch.sum(-true_dist * pred, dim=-1))
        else:
...
class KgeLoss:
    ...
    def create(config: Config):
        ...
        elif config.get("train.loss") == "bce_freq":
            offset = config.get("train.loss_arg")
            if math.isnan(offset):
                offset = 0.0
                config.set("train.loss_arg", offset, log=True)
        ...
        elif config.get("train.loss") == "kl_freq":
            return KLDivWithSoftmaxKgeLoss(config)
    ...
    def __call__(self, scores, labels, **kwargs):
        ...
        if self._bce_type is None:
            ...
            losses_negatives = (
                ...
                kwargs["label_freqs"] * losses_negatives
            ).sum(dim=1)
...
class KLDivWithSoftmaxKgeLoss(KgeLoss):
    def __init__(self, config, reduction="sum", **kwargs):
        ...
        if self._train_type == "1vsAll":
            self._celoss = LabelSmoothingLoss()
        else:
    ...
    def __call__(self, scores, labels, **kwargs):
        if labels.dim() == 1:
            ...
            if "label_freqs" in kwargs:
                return self._celoss(scores, labels, label_freqs=kwargs["label_freqs"])
            else:
