...
class Dataset(Configurable):
    ...
    def __init__(self, config, folder=None):
        ...
        #: frequencies for each part in triples.
        self._freqs: Dict[str, Dict[str, Tensor]] = {}
        self._tuple2id: Dict[str, Dict[str, Dict[(Tensor, Tensor), int]]] = {}
    ...
    def _calculate_frequencies(self, key: str):
        # calculate frequencies
        if self.config.get("train.loss") == "kl_freq" or self.config.get("train.loss") == "bce_freq":
            self._freqs[key] = {
                "s" : torch.zeros(self.num_entities()),
                "_po" : None,
                "o" : torch.zeros(self.num_entities()),
                "sp_" : None,
                "p" : torch.zeros(self.num_relations()),
                "s_o" : None,
                "all" : len(self._triples[key])}
            self._tuple2id[key] = {
                "sp_" : {},
                "_po" : {},
                "s_o" : {}}
            # create id dictionaries for relation tuples
            for torch_triple in self._triples[key]:
                triple = [int(elem) for elem in torch_triple]
                # _po
                po = (triple[1],triple[2])
                if po not in self._tuple2id[key]["_po"]:
                    self._tuple2id[key]["_po"][po] = len(self._tuple2id[key]["_po"])
                # sp_
                sp = (triple[0],triple[1])
                if sp not in self._tuple2id[key]["sp_"]:
                    self._tuple2id[key]["sp_"][sp] = len(self._tuple2id[key]["sp_"])
                # s_o
                so = (triple[0],triple[2])
                if so not in self._tuple2id[key]["s_o"]:
                    self._tuple2id[key]["s_o"][so] = len(self._tuple2id[key]["s_o"])
            self._freqs[key]["_po"] = torch.zeros(len(self._tuple2id[key]["_po"]))
            self._freqs[key]["sp_"] = torch.zeros(len(self._tuple2id[key]["sp_"]))
            self._freqs[key]["s_o"] = torch.zeros(len(self._tuple2id[key]["s_o"]))
            for torch_triple in self._triples[key]:
                triple = [int(elem) for elem in torch_triple]
                # s
                self._freqs[key]["s"][triple[0]] += 1
                # o
                self._freqs[key]["o"][triple[2]] += 1
                # p
                self._freqs[key]["p"][triple[1]] += 1
                # _po
                po = (triple[1],triple[2])
                self._freqs[key]["_po"][self._tuple2id[key]["_po"][po]] += 1
                # sp_
                sp = (triple[0],triple[1])
                self._freqs[key]["sp_"][self._tuple2id[key]["sp_"][sp]] += 1
                # s_o
                so = (triple[0],triple[2])
                self._freqs[key]["s_o"][self._tuple2id[key]["s_o"][so]] += 1
    ...
    def load_triples(self, key: str) -> Tensor:
        ...
        if key not in self._triples:
            ...
            self._calculate_frequencies(key)
